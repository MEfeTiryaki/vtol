...
void DoubleRotorGazeboPlugin::readParameters(sdf::ElementPtr sdf)
{
  ...
  if(debug_){std::cout<<"base link name : " << baseLinkName_ << std::endl;}

  topLinkName_  = sdf->GetElement("top_link_name")->Get<std::string>();
  if(debug_){std::cout<<"top link name : " << topLinkName_ << std::endl;}

  bottomLinkName_  = sdf->GetElement("bottom_link_name")->Get<std::string>();
  if(debug_){std::cout<<"bottom link name : " << bottomLinkName_ << std::endl;}
  ...
  if(debug_){std::cout<<"top joint name : " << topJointName_ << std::endl;}

  bottomJointName_  = sdf->GetElement("bottom_joint_name")->Get<std::string>();
  if(debug_){std::cout<<"bottom joint name : " << bottomJointName_ << std::endl;}
  ...
  if(debug_){std::cout<<"Drag Coefficient : " << dragCoefficient_ << std::endl;}

  reductionCoefficient_  = sdf->GetElement("second_blade_reduction")->Get<double>();
  if(debug_){std::cout<<"Second Blade Reduction : " << reductionCoefficient_ << std::endl;}
...
{
  topCommandSubscriber_= nodeHandle_->subscribe("/"+robotName_+"/"+ rotorName_+"/top/command",
                                                        10,
                                                        &DoubleRotorGazeboPlugin::TopCommandsCallback,
                                                        this);
  bottomCommandSubscriber_ = nodeHandle_->subscribe("/"+robotName_+"/"+ rotorName_+"/bottom/command",
                                                        10,
                                                        &DoubleRotorGazeboPlugin::BottomCommandsCallback,
                                                        this);
  topCommand_.data = 0;
...
void DoubleRotorGazeboPlugin::writeSimulation()
{
  ...
  // APPLY DRAG
  torque_ = thrustCoefficient_ * reductionCoefficient_ * dragCoefficient_
          * topCommand_.data * topCommand_.data ;
  torque_ += thrustCoefficient_ * reductionCoefficient_ * dragCoefficient_
          * bottomCommand_.data * bottomCommand_.data ;
...
void DoubleRotorGazeboPlugin::readSimulation()
{
  ...
  T_rotor_top_blade_.setOrigin(tf::Vector3(orientationRotorToTopBladeMath.pos.x
                                      ,orientationRotorToTopBladeMath.pos.y
                                      ,orientationRotorToTopBladeMath.pos.z));
  T_rotor_top_blade_.setRotation(tf::Quaternion(orientationRotorToTopBladeMath.rot.x
                                           ,orientationRotorToTopBladeMath.rot.y
                                           ,orientationRotorToTopBladeMath.rot.z
                                           ,orientationRotorToTopBladeMath.rot.w));

  T_rotor_bottom_blade_.setOrigin(tf::Vector3(orientationRotorToBottomBladeMath.pos.x
                                     ,orientationRotorToBottomBladeMath.pos.y
                                     ,orientationRotorToBottomBladeMath.pos.z));
  T_rotor_bottom_blade_.setRotation(tf::Quaternion(orientationRotorToBottomBladeMath.rot.x
...
void DoubleRotorGazeboPlugin::publishTF(){
  ...
  br.sendTransform(tf::StampedTransform(
                    T_rotor_top_blade_
                  , ros::Time::now()
                  , topLink_->GetParentJointsLinks().at(0)->GetName()
                  , topLinkName_));
  br.sendTransform(tf::StampedTransform(
                    T_rotor_bottom_blade_
                  , ros::Time::now()
                  , topLink_->GetParentJointsLinks().at(0)->GetName()
                  , bottomLinkName_));
